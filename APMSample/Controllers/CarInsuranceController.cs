...
public class CarInsuranceController : ControllerBase
{
    ...
    {
        // 1. 產生符合保險業規則的要保書號碼 (格式: 13C + 年月日 + 流水號)
        // 範例: 13C115010700088
        var proposalId = $"13C{DateTime.Now:yyMMdd}{Random.Shared.Next(10000, 99999)}";

        // ========================================================================
        // 【關鍵技術】使用 LogContext 建立「Log 範圍」
        // 在這個 using區塊內，所有的 Log (包含 Exception) 都會自動多一個 "ProposalId" 欄位
        // ========================================================================
        using (LogContext.PushProperty("ProposalId", proposalId))
        {
            _logger.LogInformation("收到投保請求，開始處理。車主: {Owner}, 車牌: {Plate}", request.CarOwnerName, request.CarPlate);

            try
            {
                // 讓 APM 紀錄目前的 Transaction 標籤，方便搜尋
                Agent.Tracer.CurrentTransaction?.SetLabel("ProposalId", proposalId);

                // --- 步驟 1: 驗證規則 (模擬耗時) ---
                // 使用 CaptureSpan 可以在 APM 效能圖上畫出一條獨立的 bar
            ...
            {
                // 這是業務邏輯錯誤 (例如拒保)，通常記 Warning 即可，不需要驚動 DevOps
                _logger.LogWarning("投保檢核失敗: {Reason}", bex.Message);
                return BadRequest(new { ProposalId = proposalId, Error = bex.Message });
            }
            catch (Exception ex)
            {
                // 這是系統嚴重錯誤，記 Error
                _logger.LogError(ex, "投保過程發生系統例外！要保號: {ProposalId}", proposalId);
                return StatusCode(500, new { ProposalId = proposalId, Error = "系統忙碌中，請稍後再試" });
            }
        }
    }
}
